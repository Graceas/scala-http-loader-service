...
trait HandleErrorsDirectives extends Directives with JsonFormatter with Logging {
  ...
  object ErrorFormatter {
    def apply(f: ErrorFormatter): ErrorFormatter = f
  }
  ...
  def DefaultErrorFormatter: ErrorFormatter = {
    ...
    case e: Throwable =>
      serialize(Map(
        ...
        "stack"   -> ExceptionUtils.getStackFrames(e).toList.mkString("\n"),
  ...
  def handleErrors(formatter: ErrorFormatter, headers: Seq[HttpHeader]): Directive0 =
    handleExceptions(customExceptionHandler(formatter, headers)) &
        handleRejections(customRejectionHandler) &
        respondWithDefaultHeader(RawHeader(ErrorHandlerHeader, "1"))

  private def customExceptionHandler(formatter: ErrorFormatter, headers: Seq[HttpHeader]) = {
    ExceptionHandler {
      case e: ErrorMessage =>
