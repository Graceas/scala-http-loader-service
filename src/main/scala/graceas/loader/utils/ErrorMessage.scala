...
object UnrecoverableFailures {
  def contains(e: Throwable): Boolean = e match {
    case null => false
...
object Check {
  def req(cond: Boolean, message: String): Unit = require(cond, throw new BadRequestError(message))

  def allow(cond: Boolean, message: String): Unit = require(cond, throw new ForbiddenError(message.toString))

  def allowFor(userId: String, ownerId: String, message: String): Unit =
    require(userId != null && ownerId == userId, throw new ForbiddenError(message))

  def allowFor(userId: String, ownerId1: String, ownerId2: String, message: String): Unit =
    require(userId != null && (ownerId1 == userId || ownerId2 == userId), throw new ForbiddenError(message))

  def allowIfNonEmpty(userId: String, ownerId: String, message: String): Unit =
    require(userId == null || ownerId == userId, throw new ForbiddenError(message))

  def allowIfNonEmpty(userId: String, ownerId1: String, ownerId2: String, message: String): Unit =
    require(userId == null || ownerId1 == userId || ownerId2 == userId, throw new ForbiddenError(message))
}